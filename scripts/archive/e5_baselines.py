...
"""
E5 Baseline Models: Temporal Ridge + GradBoost on 60s time-based H10 data.

Phase 1 baselines for E5 time-bin sweep. Targets H10 at 60-second bins
(10-minute returns, IC=0.248 from Phase 0 sweep).

Key differences from E4 baselines:
- Target: H10 (index 0) at 60s bins = 10 minutes (not H60 at 5s = 5 min)
- Fewer sequences: 56K total (vs 1.4M at 5s bins)
- Higher IC expected: sweep showed IC=0.248 at H10 (3x E4's H60=0.082)

Decision gates:
    B1: Best model IC > 0.08 (profitability threshold)
    B2: TemporalRidge IC > single-feature IC (confirms temporal features add value)
    B3: IC stability ratio > 2.0 (signal consistency)

Usage:
    python scripts/e5_baselines.py \
        --data-dir ../data/exports/e5_timebased_60s \
        --horizon-idx 0

Reference: EXPERIMENT_INDEX.md § E5, E5_SWEEP_RESULTS_2026_03.md
"""
...
warn_if_not_orchestrated(
    script_name="e5_baselines.py",
    suggestion=(
        "Use 'hft-ops sweep run experiments/sweeps/e5_baselines.yaml' once "
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  E5 BASELINE MODELS — {horizon_name}")
    ...
    # Compute single-feature IC (DEPTH_NORM_OFI) for comparison
    from scipy.stats import spearmanr as _spearmanr
    single_feat_vals = test_seqs[:, -1, DEPTH_NORM_OFI].astype(np.float64)
    single_feat_ic_val, _ = _spearmanr(single_feat_vals, test_labels)
    single_feat_ic_val = float(single_feat_ic_val) if np.isfinite(single_feat_ic_val) else 0.0

    # Decision gates (E5: profitability-focused)
    print(f"\n  Single-feature DEPTH_NORM_OFI IC: {single_feat_ic_val:.4f}")
    print(f"\n  Decision Gates (E5):")
    b1 = ic > 0.08  # profitability threshold
    b2 = ic > single_feat_ic_val  # temporal features add value
    gb_ic = results["models"].get("temporal_gradboost", {}).get("ic", 0)
    b3 = gb_ic > single_feat_ic_val
    print(f"    B1: Best model IC > 0.08 (profitability) → {'PASS' if b1 else 'FAIL'} (IC={ic:.4f})")
    print(f"    B2: TemporalRidge IC > single-feat IC ({single_feat_ic_val:.4f}) → {'PASS' if b2 else 'FAIL'} (IC={ic:.4f})")
    print(f"    B3: GradBoost IC > single-feat IC → {'PASS' if b3 else 'FAIL'} (IC={gb_ic:.4f})")

    results["single_feature_ic"] = single_feat_ic_val
    results["decision_gates"] = {
        "B1_profitability_ic": {"threshold": 0.08, "value": ic, "passed": bool(b1)},
        "B2_temporal_vs_single": {"threshold": single_feat_ic_val, "value": ic, "passed": bool(b2)},
        "B3_gradboost_vs_single": {"threshold": single_feat_ic_val, "value": gb_ic, "passed": bool(b3)},
    }
    ...
    if args.output_dir:
        ...
        out_file = args.output_dir / f"e5_baselines_{horizon_name}.json"
